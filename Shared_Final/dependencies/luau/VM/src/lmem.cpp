...
/*
 * MisakoRVM heap uses a size-segregated page structure, with individual pages and large allocations
 ...
 *
 * On top of this, MisakoRVM implements heap storage which is split into two types of allocations:
 *
...
/*
 * The sizes of MisakoRVM objects aren't crucial for code correctness, but they are crucial for memory efficiency
...
l_noret luaM_toobig(misako_State* L)
{
    luaG_runerror(L, "memory allocation error: block too big");
}

static lua_Page* newpage(misako_State* L, lua_Page** gcopageset, int pageSize, int blockSize, int blockCount)
{
...
static lua_Page* newclasspage(misako_State* L, lua_Page** freepageset, lua_Page** gcopageset, uint8_t sizeClass, bool storeMetadata)
{
...
static void freepage(misako_State* L, lua_Page** gcopageset, lua_Page* page)
{
    global_State* g = L->global;
...
static void freeclasspage(misako_State* L, lua_Page** freepageset, lua_Page** gcopageset, lua_Page* page, uint8_t sizeClass)
{
...
static void* newblock(misako_State* L, int sizeClass)
{
    global_State* g = L->global;
...
static void* newgcoblock(misako_State* L, int sizeClass)
{
    global_State* g = L->global;
...
static void freeblock(misako_State* L, int sizeClass, void* block)
{
    global_State* g = L->global;
...
static void freegcoblock(misako_State* L, int sizeClass, void* block, lua_Page* page)
{
    LUAU_ASSERT(page && page->busyBlocks > 0);
    LUAU_ASSERT(page->blockSize == kSizeClassConfig.sizeOfClass[sizeClass]);
    LUAU_ASSERT(block >= page->data && block < (char*)page + page->pageSize);
...
void* luaM_new_(misako_State* L, size_t nsize, uint8_t memcat)
{
    global_State* g = L->global;
...
GCObject* luaM_newgco_(misako_State* L, size_t nsize, uint8_t memcat)
{
...
void luaM_free_(misako_State* L, void* block, size_t osize, uint8_t memcat)
{
    global_State* g = L->global;
...
void luaM_freegco_(misako_State* L, GCObject* block, size_t osize, uint8_t memcat, lua_Page* page)
{
    global_State* g = L->global;
    LUAU_ASSERT((osize == 0) == (block == NULL));
...
void* luaM_realloc_(misako_State* L, void* block, size_t osize, size_t nsize, uint8_t memcat)
{
    global_State* g = L->global;
    LUAU_ASSERT((osize == 0) == (block == NULL));
...
void luaM_visitgco(misako_State* L, void* context, bool (*visitor)(void* context, lua_Page* page, GCObject* gco))
{
    global_State* g = L->global;
