...
// NOTE: If debugging the MisakoRVM code, disable this macro to prevent timeouts from
...
#define VM_INTERRUPT() \
    { \
        void (*interrupt)(misako_State*, int) = L->global->cb.interrupt; \
...
/**
 * These macros help dispatching MisakoRVM opcodes using either case
...
LUAU_NOINLINE static void luau_prepareFORN(misako_State* L, StkId plimit, StkId pstep, StkId pinit)
{
...
LUAU_NOINLINE static bool luau_loopFORG(misako_State* L, int a, int c)
{
...
LUAU_NOINLINE static void luau_callTM(misako_State* L, int nparams, int res)
{
    ++L->nCcalls;
...
LUAU_NOINLINE static void luau_tryfuncTM(misako_State* L, StkId func)
{
...
LUAU_NOINLINE void luau_callhook(misako_State* L, lua_Hook hook, void* userdata)
{
...
#define LUAU_INSN_OP decode_opcode // bitchass bogie ask me for source before you push updates so I dont skip out shit like this -Rexi
...
static void luau_execute(misako_State* L)
{
#if VM_USE_CGOTO
    ...
    LUAU_ASSERT(!FFlag::MisakoRVMNoSleepBit || !isblack(obj2gco(L))); // we don't use luaC_threadbarrier because active threads never turn black
...
void luau_execute(misako_State* L)
{
    if (L->singlestep)
        luau_execute<true>(L);
    else
...
int luau_precall(misako_State* L, StkId func, int nresults)
{
...
void luau_poscall(misako_State* L, StkId first)
{
