...
static const char* currfuncname(misako_State* L)
{
...
l_noret luaL_argerrorL(misako_State* L, int narg, const char* extramsg)
{
...
l_noret luaL_typeerrorL(misako_State* L, int narg, const char* tname)
{
    const char* fname = currfuncname(L);
...
static l_noret tag_error(misako_State* L, int narg, int tag)
{
    luaL_typeerrorL(L, narg, lua_typename(L, tag));
}

void luaL_where(misako_State* L, int level)
{
    lua_Debug ar;
...
l_noret luaL_errorL(misako_State* L, const char* fmt, ...)
{
    va_list argp;
    va_start(argp, fmt);
    luaL_where(L, 1);
...
int luaL_checkoption(misako_State* L, int narg, const char* def, const char* const lst[])
{
...
int luaL_newmetatable(misako_State* L, const char* tname)
{
...
void* luaL_checkudata(misako_State* L, int ud, const char* tname)
{
...
void luaL_checkstack(misako_State* L, int space, const char* mes)
{
    if (!lua_checkstack(L, space))
        luaL_error(L, "stack overflow (%s)", mes);
}

void luaL_checktype(misako_State* L, int narg, int t)
{
    if (lua_type(L, narg) != t)
        tag_error(L, narg, t);
}

void luaL_checkany(misako_State* L, int narg)
{
    if (lua_type(L, narg) == LUA_TNONE)
        luaL_error(L, "missing argument #%d", narg);
}

const char* luaL_checklstring(misako_State* L, int narg, size_t* len)
{
...
const char* luaL_optlstring(misako_State* L, int narg, const char* def, size_t* len)
{
...
double luaL_checknumber(misako_State* L, int narg)
{
    int isnum;
...
double luaL_optnumber(misako_State* L, int narg, double def)
{
    return luaL_opt(L, luaL_checknumber, narg, def);
}

int luaL_checkboolean(misako_State* L, int narg)
{
...
int luaL_optboolean(misako_State* L, int narg, int def)
{
    return luaL_opt(L, luaL_checkboolean, narg, def);
}

int luaL_checkinteger(misako_State* L, int narg)
{
    int isnum;
...
int luaL_optinteger(misako_State* L, int narg, int def)
{
    return luaL_opt(L, luaL_checkinteger, narg, def);
}

unsigned luaL_checkunsigned(misako_State* L, int narg)
{
    int isnum;
...
unsigned luaL_optunsigned(misako_State* L, int narg, unsigned def)
{
    return luaL_opt(L, luaL_checkunsigned, narg, def);
}

const float* luaL_checkvector(misako_State* L, int narg)
{
...
const float* luaL_optvector(misako_State* L, int narg, const float* def)
{
    return luaL_opt(L, luaL_checkvector, narg, def);
}

int luaL_getmetafield(misako_State* L, int obj, const char* event)
{
...
int luaL_callmeta(misako_State* L, int obj, const char* event)
{
    obj = abs_index(L, obj);
...
void luaL_register(misako_State* L, const char* libname, const luaL_Reg* l)
{
    if (libname)
    {
        int size = libsize(l);
...
const char* luaL_findtable(misako_State* L, int idx, const char* fname, int szhint)
{
    const char* e;
    lua_pushvalue(L, idx);
    do
...
const char* luaL_typename(misako_State* L, int idx)
{
...
static size_t getnextbuffersize(misako_State* L, size_t currentsize, size_t desiredsize)
{
...
void luaL_buffinit(misako_State* L, luaL_Buffer* B)
{
...
char* luaL_buffinitsize(misako_State* L, luaL_Buffer* B, size_t size)
{
    luaL_buffinit(L, B);
...
const char* luaL_tolstring(misako_State* L, int idx, size_t* len)
{
