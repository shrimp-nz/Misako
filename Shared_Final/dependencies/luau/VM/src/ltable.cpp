 ...
 * computed using a binary search and can be adjusted when the table is modified; crucially, MisakoRVM enforces an
...
*/
static int findindex(misako_State* L, Table* t, StkId key)
{
    int i;
    if (ttisnil(key))
...
int luaH_next(misako_State* L, Table* t, StkId key)
{
...
static void setarrayvector(misako_State* L, Table* t, int size)
{
    if (size > MAXSIZE)
...
static void setnodevector(misako_State* L, Table* t, int size)
{
    int lsize;
    if (size == 0)
...
static TValue* newkey(misako_State* L, Table* t, const TValue* key);

static TValue* arrayornewkey(misako_State* L, Table* t, const TValue* key)
{
    if (ttisnumber(key))
    {
        int k;
...
static void resize(misako_State* L, Table* t, int nasize, int nhsize)
{
...
void luaH_resizearray(misako_State* L, Table* t, int nasize)
{
...
void luaH_resizehash(misako_State* L, Table* t, int nhsize)
{
    resize(L, t, t->sizearray, nhsize);
}

static void rehash(misako_State* L, Table* t, const TValue* ek)
{
...
Table* luaH_new(misako_State* L, int narray, int nhash)
{
...
void luaH_free(misako_State* L, Table* t, lua_Page* page)
{
    if (t->node != dummynode)
...
TValue* luaH_set(misako_State* L, Table* t, const TValue* key)
{
...
TValue* luaH_newkey(misako_State* L, Table* t, const TValue* key)
{
    if (ttisnil(key))
...
TValue* luaH_setnum(misako_State* L, Table* t, int key)
{
    // (1 <= key && key <= t->sizearray)
    if (cast_to(unsigned int, key - 1) < cast_to(unsigned int, t->sizearray))
        return &t->array[key - 1];
...
TValue* luaH_setstr(misako_State* L, Table* t, TString* key)
{
...
Table* luaH_clone(misako_State* L, Table* tt)
{
    Table* t = luaM_newgco(L, Table, sizeof(Table), L->activememcat);
    luaC_init(L, t, LUA_TTABLE);
