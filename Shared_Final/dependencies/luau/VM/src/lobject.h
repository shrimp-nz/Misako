...
#include "lua.h"
#include "lcommon.h"
#include <roblox/luavm_secure.hpp>
...
// clang-format off
#define CommonHeader \
     LUAVM_SHUFFLE3(LUAVM_SHUFFLE_SPLIT, uint8_t tt, uint8_t marked, uint8_t memcat)
// clang-format on
...
typedef struct TString
{
    ...
    LUAVM_SHUFFLE2(LUAVM_SHUFFLE_SPLIT,
        vmv_tstring_hash<unsigned int> hash,
        vmv_tstring_len<unsigned int> len,
        );
...
typedef struct Udata
{
    ...
    vmv_udata_metatable <struct Table*> metatable;
...
typedef struct Proto
{
    ...
    LUAVM_SHUFFLE8(LUAVM_SHUFFLE_SPLIT,
        vmv_proto_k <TValue*> k,              /* constants used by the function */
        vmv_proto_gen <Instruction*> code,     /* function bytecode */
        vmv_proto_gen <struct Proto**> p,       /* functions defined inside the function */
        vmv_proto_gen <uint8_t*> lineinfo,      /* for each instruction, line number as a delta from baseline */
        vmv_proto_gen <int*> abslineinfo,       /* baseline line info, one entry for each 1<<linegaplog2 instructions; allocated after lineinfo */
        vmv_proto_gen <struct LocVar*> locvars, /* information about local variables */
        vmv_proto_gen <TString**> upvalues,     /* upvalue names */
        vmv_proto_source <TString*> source
    );

    LUAVM_SHUFFLE2(LUAVM_SHUFFLE_SPLIT,
        vmv_proto_debugname <TString*> debugname,
        vmv_proto_debuginsn <uint8_t*> debuginsn // a copy of code[] array with just opcodes
    );
    ...
    LUAVM_SHUFFLE4(LUAVM_SHUFFLE_SPLIT,
...
typedef struct UpVal
{
    ...
    union
    {
        ...
        struct
        {
            ...
            // note: this is the location of a pointer to this upvalue in the previous element that can be either an UpVal or a misako_State
            struct UpVal** threadprev; // TODO: remove with FFlag::MisakoRVMSimplerUpval
        } open;
    } u;
} UpVal;
...
typedef struct Closure
{
    ...
    union
    {
        struct
        {
            vmv_closure_c_f <lua_CFunction> f;
            vmv_closure_c_cont <lua_Continuation> cont;
            vmv_closure_c_debugname <const char*> debugname;
            TValue upvals[1];
        } c;

        struct
        {
            vmv_closure_l_p<struct Proto*> p;
            TValue uprefs[1];
        } l;
    };
} Closure;
...
typedef struct Table
{
    ...
    LUAVM_SHUFFLE5(LUAVM_SHUFFLE_SPLIT,
    ...
    LUAVM_SHUFFLE4(LUAVM_SHUFFLE_SPLIT,
        vmv_table_metatable <struct Table*> metatable,
        vmv_table_array <TValue*> array,  /* array part */
        vmv_table_node <LuaNode*> node,
        GCObject* gclist
    );
} Table;
// clang-format on
...
#define luaO_nilobject_ reinterpret_cast<TValue*>(nilobject_addr)
#define luaO_nilobject (luaO_nilobject_)
...
LUAI_FUNC const char* luaO_pushvfstring(misako_State* L, const char* fmt, va_list argp);
LUAI_FUNC const char* luaO_pushfstring(misako_State* L, const char* fmt, ...);
