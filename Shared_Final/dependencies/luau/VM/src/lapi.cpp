...
LUAU_FASTFLAG(MisakoRVMSimplerUpval)

const char* lua_ident = "$Misako: Misako 1.0 Copyright (C) 1994-2008 misako.tech, Misako $\n"
"$Authors: Rexi, Bogie, Mellon and Team ScriptWare $\n"
"$URL: www.misako.tech $\n";

const char* luau_ident = "$MisakoRVM: Copyright (C) 2021-2022 Misako Corporation $\n"
"$URL: misako.tech $\n";
...
static Table* getcurrenv(misako_State* L)
{
...
static LUAU_NOINLINE TValue* pseudo2addr(misako_State* L, int idx)
{
...
LUAU_FORCEINLINE TValue* index2addr(misako_State* L, int idx)
{
    if (idx > 0)
    {
...
const TValue* luaA_toobject(misako_State* L, int idx)
{
    StkId p = index2addr(L, idx);
    return (p == luaO_nilobject) ? NULL : p;
}

void luaA_pushobject(misako_State* L, const TValue* o)
{
    setobj2s(L, L->top, o);
    api_incr_top(L);
}

int lua_checkstack(misako_State* L, int size)
{
    int res = 1;
...
void lua_rawcheckstack(misako_State* L, int size)
{
    luaD_checkstack(L, size);
    expandstacklimit(L, L->top + size);
    return;
...
void lua_xmove(misako_State* from, misako_State* to, int n)
{
    if (from == to)
        return;
...
void lua_xpush(misako_State* from, misako_State* to, int idx)
{
    api_check(from, from->global == to->global);
    luaC_threadbarrier(to);
...
misako_State* lua_newthread(misako_State* L)
{
    luaC_checkGC(L);
    luaC_threadbarrier(L);
    misako_State* L1 = luaE_newthread(L);
...
misako_State* lua_mainthread(misako_State* L)
{
...
int lua_absindex(misako_State* L, int idx)
{
...
int lua_gettop(misako_State* L)
{
    return cast_int(L->top - L->base);
}

void lua_settop(misako_State* L, int idx)
{
    if (idx >= 0)
    {
...
void lua_remove(misako_State* L, int idx)
{
    StkId p = index2addr(L, idx);
...
void lua_insert(misako_State* L, int idx)
{
    luaC_threadbarrier(L);
    StkId p = index2addr(L, idx);
    api_checkvalidindex(L, p);
...
void lua_replace(misako_State* L, int idx)
{
    api_checknelems(L, 1);
    luaC_threadbarrier(L);
...
void lua_pushvalue(misako_State* L, int idx)
{
    luaC_threadbarrier(L);
    StkId o = index2addr(L, idx);
    setobj2s(L, L->top, o);
...
int lua_type(misako_State* L, int idx)
{
    StkId o = index2addr(L, idx);
    return (o == luaO_nilobject) ? LUA_TNONE : ttype(o);
}

const char* lua_typename(misako_State* L, int t)
{
    return (t == LUA_TNONE) ? "no value" : luaT_typenames[t];
}

int lua_iscfunction(misako_State* L, int idx)
{
    StkId o = index2addr(L, idx);
    return iscfunction(o);
}

int lua_isLfunction(misako_State* L, int idx)
{
    StkId o = index2addr(L, idx);
    return isLfunction(o);
}

int lua_isnumber(misako_State* L, int idx)
{
    TValue n;
...
int lua_isstring(misako_State* L, int idx)
{
    int t = lua_type(L, idx);
    return (t == LUA_TSTRING || t == LUA_TNUMBER);
}

int lua_isuserdata(misako_State* L, int idx)
{
    const TValue* o = index2addr(L, idx);
    return (ttisuserdata(o) || ttislightuserdata(o));
}

int lua_rawequal(misako_State* L, int index1, int index2)
{
...
int lua_equal(misako_State* L, int index1, int index2)
{
    StkId o1, o2;
    int i;
...
int lua_lessthan(misako_State* L, int index1, int index2)
{
    StkId o1, o2;
    int i;
    o1 = index2addr(L, index1);
...
double lua_tonumberx(misako_State* L, int idx, int* isnum)
{
    TValue n;
    const TValue* o = index2addr(L, idx);
    if (tonumber(o, &n))
...
int lua_tointegerx(misako_State* L, int idx, int* isnum)
{
    TValue n;
    const TValue* o = index2addr(L, idx);
    if (tonumber(o, &n))
...
unsigned lua_tounsignedx(misako_State* L, int idx, int* isnum)
{
    TValue n;
    const TValue* o = index2addr(L, idx);
    if (tonumber(o, &n))
...
int lua_toboolean(misako_State* L, int idx)
{
    const TValue* o = index2addr(L, idx);
    return !l_isfalse(o);
}

const char* lua_tolstring(misako_State* L, int idx, size_t* len)
{
    StkId o = index2addr(L, idx);
    if (!ttisstring(o))
    {
...
const char* lua_tostringatom(misako_State* L, int idx, int* atom)
{
    StkId o = index2addr(L, idx);
    if (!ttisstring(o))
        return NULL;
...
const char* lua_namecallatom(misako_State* L, int* atom)
{
...
const float* lua_tovector(misako_State* L, int idx)
{
    StkId o = index2addr(L, idx);
    if (!ttisvector(o))
        return NULL;
...
int lua_objlen(misako_State* L, int idx)
{
    StkId o = index2addr(L, idx);
    switch (ttype(o))
    {
...
lua_CFunction lua_tocfunction(misako_State* L, int idx)
{
    StkId o = index2addr(L, idx);
    return (!iscfunction(o)) ? NULL : cast_to(lua_CFunction, clvalue(o)->c.f);
}

void* lua_tolightuserdata(misako_State* L, int idx)
{
    StkId o = index2addr(L, idx);
    return (!ttislightuserdata(o)) ? NULL : pvalue(o);
}

void* lua_touserdata(misako_State* L, int idx)
{
    StkId o = index2addr(L, idx);
    if (ttisuserdata(o))
...
void* lua_touserdatatagged(misako_State* L, int idx, int tag)
{
    StkId o = index2addr(L, idx);
    return (ttisuserdata(o) && uvalue(o)->tag == tag) ? uvalue(o)->data : NULL;
}

int lua_userdatatag(misako_State* L, int idx)
{
    StkId o = index2addr(L, idx);
    if (ttisuserdata(o))
        return uvalue(o)->tag;
...
misako_State* lua_tothread(misako_State* L, int idx)
{
    StkId o = index2addr(L, idx);
    return (!ttisthread(o)) ? NULL : thvalue(o);
}

const void* lua_topointer(misako_State* L, int idx)
{
    StkId o = index2addr(L, idx);
    switch (ttype(o))
    {
...
void lua_pushnil(misako_State* L)
{
    setnilvalue(L->top);
    api_incr_top(L);
    return;
...
void lua_pushnumber(misako_State* L, double n)
{
    setnvalue(L->top, n);
    api_incr_top(L);
    return;
...
void lua_pushinteger(misako_State* L, int n)
{
...
void lua_pushunsigned(misako_State* L, unsigned u)
{
...
void lua_pushvector(misako_State* L, float x, float y, float z, float w)
{
...
#else
void lua_pushvector(misako_State* L, float x, float y, float z)
{
...
void lua_pushlstring(misako_State* L, const char* s, size_t len)
{
    luaC_checkGC(L);
    luaC_threadbarrier(L);
...
void lua_pushstring(misako_State* L, const char* s)
{
    if (s == NULL)
        lua_pushnil(L);
    else
...
const char* lua_pushvfstring(misako_State* L, const char* fmt, va_list argp)
{
    luaC_checkGC(L);
    luaC_threadbarrier(L);
...
const char* lua_pushfstringL(misako_State* L, const char* fmt, ...)
{
    luaC_checkGC(L);
    luaC_threadbarrier(L);
    va_list argp;
...
void lua_pushcclosurek(misako_State* L, lua_CFunction fn, const char* debugname, int nup, lua_Continuation cont)
{
    luaC_checkGC(L);
    luaC_threadbarrier(L);
...
void lua_pushboolean(misako_State* L, int b)
{
...
void lua_pushlightuserdata(misako_State* L, void* p)
{
    setpvalue(L->top, p);
    api_incr_top(L);
    return;
...
int lua_pushthread(misako_State* L)
{
    luaC_threadbarrier(L);
...
int lua_gettable(misako_State* L, int idx)
{
    luaC_threadbarrier(L);
...
int lua_getfield(misako_State* L, int idx, const char* k)
{
    luaC_threadbarrier(L);
    StkId t = index2addr(L, idx);
    api_checkvalidindex(L, t);
...
int lua_rawgetfield(misako_State* L, int idx, const char* k)
{
    luaC_threadbarrier(L);
    StkId t = index2addr(L, idx);
...
int lua_rawget(misako_State* L, int idx)
{
    luaC_threadbarrier(L);
    StkId t = index2addr(L, idx);
    api_check(L, ttistable(t));
...
int lua_rawgeti(misako_State* L, int idx, int n)
{
    luaC_threadbarrier(L);
    StkId t = index2addr(L, idx);
    api_check(L, ttistable(t));
...
void lua_createtable(misako_State* L, int narray, int nrec)
{
    luaC_checkGC(L);
    luaC_threadbarrier(L);
...
void lua_setreadonly(misako_State* L, int objindex, int enabled)
{
...
int lua_getreadonly(misako_State* L, int objindex)
{
    const TValue* o = index2addr(L, objindex);
    api_check(L, ttistable(o));
    Table* t = hvalue(o);
...
void lua_setsafeenv(misako_State* L, int objindex, int enabled)
{
    const TValue* o = index2addr(L, objindex);
    api_check(L, ttistable(o));
    Table* t = hvalue(o);
...
int lua_getmetatable(misako_State* L, int objindex)
{
    luaC_threadbarrier(L);
    Table* mt = NULL;
...
void lua_getfenv(misako_State* L, int idx)
{
    luaC_threadbarrier(L);
    StkId o = index2addr(L, idx);
    api_checkvalidindex(L, o);
...
void lua_settable(misako_State* L, int idx)
{
    api_checknelems(L, 2);
    StkId t = index2addr(L, idx);
    api_checkvalidindex(L, t);
...
void lua_setfield(misako_State* L, int idx, const char* k)
{
    api_checknelems(L, 1);
    StkId t = index2addr(L, idx);
    api_checkvalidindex(L, t);
...
void lua_rawset(misako_State* L, int idx)
{
    api_checknelems(L, 2);
    StkId t = index2addr(L, idx);
    api_check(L, ttistable(t));
...
void lua_rawseti(misako_State* L, int idx, int n)
{
    api_checknelems(L, 1);
    StkId o = index2addr(L, idx);
    api_check(L, ttistable(o));
...
int lua_setmetatable(misako_State* L, int objindex)
{
    api_checknelems(L, 1);
...
int lua_setfenv(misako_State* L, int idx)
{
    int res = 1;
    api_checknelems(L, 1);
    StkId o = index2addr(L, idx);
...
void lua_call(misako_State* L, int nargs, int nresults)
{
    StkId func;
...
*/
static void f_call(misako_State* L, void* ud)
{
...
int lua_pcall(misako_State* L, int nargs, int nresults, int errfunc)
{
    api_checknelems(L, nargs + 1);
    api_check(L, L->status == 0);
    checkresults(L, nargs, nresults);
...
int lua_status(misako_State* L)
{
    return L->status;
}

void* lua_getthreaddata(misako_State* L)
{
    return L->userdata;
}

void lua_setthreaddata(misako_State* L, void* data)
{
    L->userdata = data;
}
...
int lua_gc(misako_State* L, int what, int data)
{
    int res = 0;
...
l_noret lua_error(misako_State* L)
{
    api_checknelems(L, 1);
...
int lua_next(misako_State* L, int idx)
{
    luaC_threadbarrier(L);
    StkId t = index2addr(L, idx);
    api_check(L, ttistable(t));
...
void lua_concat(misako_State* L, int n)
{
    api_checknelems(L, n);
    if (n >= 2)
    {
...
void* lua_newuserdatatagged(misako_State* L, size_t sz, int tag)
{
...
void* lua_newuserdatadtor(misako_State* L, size_t sz, void (*dtor)(void*))
{
    luaC_checkGC(L);
    luaC_threadbarrier(L);
...
const char* lua_getupvalue(misako_State* L, int funcindex, int n)
{
    luaC_threadbarrier(L);
    TValue* val;
...
const char* lua_setupvalue(misako_State* L, int funcindex, int n)
{
    api_checknelems(L, 1);
    ...
    if (name)
    {
        ...
        if (!FFlag::MisakoRVMSimplerUpval)
            luaC_upvalbarrier(L, cast_to(UpVal*, NULL), val);
    }
    return name;
}

uintptr_t lua_encodepointer(misako_State* L, uintptr_t p)
{
    global_State* g = L->global;
    return uintptr_t((g->ptrenckey[0] * p + g->ptrenckey[2]) ^ (g->ptrenckey[1] * p + g->ptrenckey[3]));
}

int lua_ref(misako_State* L, int idx)
{
...
void lua_unref(misako_State* L, int ref)
{
    if (ref <= LUA_REFNIL)
        return;
...
void lua_setuserdatatag(misako_State* L, int idx, int tag)
{
...
void lua_setuserdatadtor(misako_State* L, int tag, void (*dtor)(misako_State*, void*))
{
    api_check(L, unsigned(tag) < LUA_UTAG_LIMIT);
    L->global->udatagc[tag] = dtor;
}

void lua_clonefunction(misako_State* L, int idx)
{
    luaC_checkGC(L);
    luaC_threadbarrier(L);
    StkId p = index2addr(L, idx);
...
LUAU_FORCEINLINE void lua_clonecfunction(misako_State* L, int idx) //bogie go brrr dont forget to add this in future updates
{
    luaC_checkGC(L);
    luaC_threadbarrier(L);
    ...
    newcl->c.f = cl->c.f.get();
    newcl->c.cont = cl->c.cont.get();
    newcl->c.debugname = cl->c.debugname.get();
    for (int i = 0; i < cl->nupvalues; i++)
...
lua_Callbacks* lua_callbacks(misako_State* L)
{
    return &L->global->cb;
}

void lua_setmemcat(misako_State* L, int category)
{
    api_check(L, unsigned(category) < LUA_MEMORY_CATEGORIES);
    L->activememcat = uint8_t(category);
}

size_t lua_totalbytes(misako_State* L, int category)
{
