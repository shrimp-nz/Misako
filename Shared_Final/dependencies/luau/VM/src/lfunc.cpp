...
Proto* luaF_newproto(misako_State* L)
{
...
Closure* luaF_newLclosure(misako_State* L, int nelems, Table* e, Proto* p)
{
...
Closure* luaF_newCclosure(misako_State* L, int nelems, Table* e)
{
...
UpVal* luaF_findupval(misako_State* L, StkId level)
{
    ...
    {
        LUAU_ASSERT(!FFlag::MisakoRVMSimplerUpval || !isdead(g, obj2gco(p)));
        LUAU_ASSERT(upisopen(p));
        if (p->v == level)
        {                                                          // found a corresponding upvalue?
            if (!FFlag::MisakoRVMSimplerUpval && isdead(g, obj2gco(p))) // is it dead?
...
void luaF_freeupval(misako_State* L, UpVal* uv, lua_Page* page)
{
    if (!FFlag::MisakoRVMSimplerUpval && uv->v != &uv->u.value) // is it open?
        luaF_unlinkupval(uv);                              // remove from open list
    luaM_freegco(L, uv, sizeof(UpVal), uv->memcat, page);  // free upvalue
}

void luaF_close(misako_State* L, StkId level)
{
    global_State* g = L->global;
    UpVal* uv;
...
void luaF_closeupval(misako_State* L, UpVal* uv, bool dead)
{
    LUAU_ASSERT(FFlag::MisakoRVMSimplerUpval);
...
void luaF_freeproto(misako_State* L, Proto* f, lua_Page* page)
{
...
void luaF_freeclosure(misako_State* L, Closure* c, lua_Page* page)
{
