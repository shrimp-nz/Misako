...
/*
 * MisakoRVM uses an incremental non-generational non-moving mark&sweep garbage collector.
 *
 ...
 * with minimal amount of effort. To configure the pacer MisakoRVM provides control over three variables: GC goal, defined as the
 ...
 *
 * MisakoRVM uses both types of barriers. Forward barriers advance GC progress, since they don't create new outstanding work for GC,
 ...
 *
 * NOTE: The above is only true when MisakoRVMNoSleepBit is enabled.
 *
 ...
 *
 * NOTE: The above is only true when MisakoRVMSimplerUpval is enabled.
 */

LUAU_FASTFLAGVARIABLE(MisakoRVMSimplerUpval, false)
LUAU_FASTFLAGVARIABLE(MisakoRVMNoSleepBit, false)
LUAU_FASTFLAGVARIABLE(MisakoRVMEagerShrink, false)

#define GC_SWEEPPAGESTEPCOST 16

#define GC_INTERRUPT(state) \
    { \
        void (*interrupt)(misako_State*, int) = g->cb.interrupt; \
...
static void traversestack(global_State* g, misako_State* l)
{
    markobject(g, l->gt);
    if (l->namecall)
...
static void clearstack(misako_State* l)
{
...
// TODO: pull function definition here when FFlag::MisakoRVMEagerShrink is removed
static void shrinkstack(misako_State* L);
...
static size_t propagatemark(global_State* g)
{
    ...
    case LUA_TTHREAD:
    {
        ...
        // TODO: Refactor this logic after MisakoRVMNoSleepBit is removed
        if (!active && g->gcstate == GCSpropagate)
        {
            traversestack(g, th);
            clearstack(th);

            if (!FFlag::MisakoRVMNoSleepBit)
        ...
        if (FFlag::MisakoRVMEagerShrink && g->gcstate != GCSatomic)
            shrinkstack(th);

        return sizeof(misako_State) + sizeof(TValue) * th->stacksize + sizeof(CallInfo) * th->size_ci;
    }
    case LUA_TPROTO:
    {
        Proto* p = gco2p(o);
...
*/
static size_t cleartable(misako_State* L, GCObject* l)
{
    size_t work = 0;
    while (l)
    {
...
static void freeobj(misako_State* L, GCObject* o, lua_Page* page)
{
    switch (o->gch.tt)
    {
    case LUA_TPROTO:
...
static void shrinkbuffers(misako_State* L)
{
...
static void shrinkbuffersfull(misako_State* L)
{
    global_State* g = L->global;
    // check size of string hash
...
static bool deletegco(void* context, lua_Page* page, GCObject* gco)
{
    ...
    if (!FFlag::MisakoRVMSimplerUpval && gco->gch.tt == LUA_TTHREAD)
    {
        misako_State* th = gco2th(gco);
    ...
    misako_State* L = (misako_State*)context;
    freeobj(L, gco, page);
    return true;
}

void luaC_freeall(misako_State* L)
{
    global_State* g = L->global;
...
// mark root set
static void markroot(misako_State* L)
{
    global_State* g = L->global;
    g->gray = NULL;
    g->grayagain = NULL;
...
static size_t clearupvals(misako_State* L)
{
    global_State* g = L->global;
...
static size_t atomic(misako_State* L)
{
    global_State* g = L->global;
...
static bool sweepgco(misako_State* L, lua_Page* page, GCObject* gco)
{
    global_State* g = L->global;
...
static int sweepgcopage(misako_State* L, lua_Page* page)
{
    char* start;
    char* end;
    int busyBlocks;
...
static size_t gcstep(misako_State* L, size_t limit)
{
    size_t cost = 0;
    global_State* g = L->global;
    switch (g->gcstate)
...
size_t luaC_step(misako_State* L, bool assist)
{
    global_State* g = L->global;
...
void luaC_fullgc(misako_State* L)
{
    global_State* g = L->global;
    ...
    if (FFlag::MisakoRVMSimplerUpval ? keepinvariant(g) : g->gcstate <= GCSatomic)
    {
...
void luaC_barrierupval(misako_State* L, GCObject* v)
{
    LUAU_ASSERT(!FFlag::MisakoRVMSimplerUpval);
    global_State* g = L->global;
...
void luaC_barrierf(misako_State* L, GCObject* o, GCObject* v)
{
    global_State* g = L->global;
...
void luaC_barriertable(misako_State* L, Table* t, GCObject* v)
{
    global_State* g = L->global;
...
void luaC_barrierback(misako_State* L, GCObject* o, GCObject** gclist)
{
    global_State* g = L->global;
    LUAU_ASSERT(isblack(o) && !isdead(g, o));
    LUAU_ASSERT(g->gcstate != GCSpause);
...
void luaC_upvalclosed(misako_State* L, UpVal* uv)
{
    global_State* g = L->global;
...
int64_t luaC_allocationrate(misako_State* L)
{
    global_State* g = L->global;
...
void luaC_wakethread(misako_State* L)
{
    LUAU_ASSERT(!FFlag::MisakoRVMNoSleepBit);
