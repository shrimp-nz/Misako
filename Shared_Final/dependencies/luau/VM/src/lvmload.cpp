...
struct TempBuffer
{
    ...
    TempBuffer(misako_State* L, size_t count)
        : L(L)
...
void luaV_getimport(misako_State* L, Table* env, TValue* k, uint32_t id, bool propagatenil)
{
    int count = id >> 30;
...
static void resolveImportSafe(misako_State* L, Table* env, TValue* k, uint32_t id)
{
    struct ResolveImport
    {
        ...
        static void run(misako_State* L, void* ud)
        {
...
int luau_load(misako_State* L, const char* chunkname, const char* data, size_t size, int env)
{
    size_t offset = 0;
