...
int luaD_rawrunprotected(misako_State* L, Pfunc f, void* ud)
{
    lua_jmpbuf jb;
...
l_noret luaD_throw(misako_State* L, int errcode)
{
...
{
public:
    lua_exception(misako_State* L, int status)
        : L(L)
        , status(status)
    {
    }
...
int luaD_rawrunprotected(misako_State* L, Pfunc f, void* ud)
{
    ...
    {
        // MisakoRVM will never throw this, but this can catch exceptions that escape from C++ implementations of external functions
        try
        {
...
static void correctstack(misako_State* L, TValue* oldstack)
{
...
void luaD_reallocstack(misako_State* L, int newsize)
{
...
void luaD_reallocCI(misako_State* L, int newsize)
{
...
void luaD_growstack(misako_State* L, int n)
{
...
CallInfo* luaD_growCI(misako_State* L)
{
...
void luaD_checkCstack(misako_State* L)
{
    // allow extra stack space to handle stack overflow in xpcall
...
** function position.
*/
void luaD_call(misako_State* L, StkId func, int nResults)
{
    if (++L->nCcalls >= LUAI_MAXCCALLS)
        luaD_checkCstack(L);
...
static void seterrorobj(misako_State* L, int errcode, StkId oldtop)
{
    switch (errcode)
    {
    case LUA_ERRMEM:
...
static void resume_continue(misako_State* L)
{
...
static void resume(misako_State* L, void* ud)
{
...
static CallInfo* resume_findhandler(misako_State* L)
{
    CallInfo* ci = L->ci;
...
static void resume_handle(misako_State* L, void* ud)
{
...
static int resume_error(misako_State* L, const char* msg)
{
    L->top = L->ci->base;
...
static void resume_finish(misako_State* L, int status)
{
    L->nCcalls = L->baseCcalls;
    resetbit(L->stackstate, THREAD_ACTIVEBIT);
...
int lua_resume(misako_State* L, misako_State* from, int nargs)
{
    int status;
...
int lua_resumeerror(misako_State* L, misako_State* from)
{
    int status;
    if (L->status != LUA_YIELD && L->status != LUA_BREAK && (L->status != 0 || L->ci != L->base_ci))
        return resume_error(L, "cannot resume non-suspended coroutine");
...
int lua_yield(misako_State* L, int nresults)
{
...
int lua_isyieldable(misako_State* L)
{
    return (L->nCcalls <= L->baseCcalls);
}

static void callerrfunc(misako_State* L, void* ud)
{
...
static void restore_stack_limit(misako_State* L)
{
    LUAU_ASSERT(L->stack_last - L->stack == L->stacksize - EXTRA_STACK);
    if (L->size_ci > LUAI_MAXCALLS)
...
int luaD_pcall(misako_State* L, Pfunc func, void* u, ptrdiff_t old_top, ptrdiff_t ef)
{
