...
typedef struct stringtable
{
    ...
    LUAVM_SHUFFLE3(LUAVM_SHUFFLE_SPLIT,
        TString** hash,
...
typedef struct global_State
{
    ...
    TString* strbufgc; // list of all string buffer objects; TODO: remove with MisakoRVMNoStrbufLink
    ...
    LUAVM_SHUFFLE5(LUAVM_SHUFFLE_SPLIT,
        struct misako_State* mainthread,
        UpVal uvhead,                                   /* head of double-linked list of all open upvalues */
        struct Table* mt[LUA_T_COUNT],                  /* metatables for basic types */
        vmv_global_state_ttname<TString*> ttname[LUA_T_COUNT],       /* names for basic types */
        vmv_global_state_tmname<TString*> tmname[TM_N]             /* array with tag-method names */
    );
    ...
    void (*udatagc[LUA_UTAG_LIMIT])(misako_State*, void*); // for each userdata tag, a gc callback to be called immediately before freeing memory
...
struct misako_State
{
    ...
    LUAVM_SHUFFLE6(LUAVM_SHUFFLE_SPLIT,
        StkId top,                                     /* first free slot in the stack */
        StkId base,                                       /* base of current function */
        vmv_lua_state_global<global_State*> global,
    ...
    LUAVM_SHUFFLE2(LUAVM_SHUFFLE_SPLIT,
        vmv_lua_state_stacksize<int> stacksize,
    ...
    int cachedslot;    // when table operations or INDEX/NEWINDEX is invoked from MisakoRVM, what is the expected slot for lookup?
    ...
    TString* namecall; // when invoked from MisakoRVM using NAMECALL, what method do we need to invoke?
...
LUAI_FUNC misako_State* luaE_newthread(misako_State* L);
LUAI_FUNC void luaE_freethread(misako_State* L, misako_State* L1, struct lua_Page* page);
