...
static void stack_init(misako_State* L1, misako_State* L)
{
...
static void freestack(misako_State* L, misako_State* L1)
{
...
*/
static void f_luaopen(misako_State* L, void* ud)
{
...
static void preinit_state(misako_State* L, global_State* g)
{
    L->global = g;
    L->stack = NULL;
    L->stacksize = 0;
...
static void close_state(misako_State* L)
{
    global_State* g = L->global;
...
misako_State* luaE_newthread(misako_State* L)
{
    misako_State* L1 = luaM_newgco(L, misako_State, sizeof(misako_State), L->activememcat);
...
void luaE_freethread(misako_State* L, misako_State* L1, lua_Page* page)
{
    ...
    freestack(L, L1);
    luaM_freegco(L, L1, sizeof(misako_State), L1->memcat, page);
}

void lua_resetthread(misako_State* L)
{
...
int lua_isthreadreset(misako_State* L)
{
    return L->ci == L->base_ci && L->base == L->top && L->status == LUA_OK;
}

misako_State* lua_newstate(lua_Alloc f, void* ud)
{
    int i;
    misako_State* L;
    global_State* g;
