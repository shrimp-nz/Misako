...
LUAU_FASTFLAGVARIABLE(MisakoRVMNoStrbufLink, false)
...
void luaS_resize(misako_State* L, int newsize)
{
...
static TString* newlstr(misako_State* L, const char* str, size_t l, unsigned int h)
{
    if (l > MAXSSIZE)
        luaM_toobig(L);
...
static void unlinkstrbuf(misako_State* L, TString* ts)
{
    LUAU_ASSERT(!FFlag::MisakoRVMNoStrbufLink);
...
TString* luaS_bufstart(misako_State* L, size_t size)
{
    if (size > MAXSSIZE)
        luaM_toobig(L);
    ...
    if (FFlag::MisakoRVMNoStrbufLink)
    {
        ts->next = NULL;
    }
    else
...
TString* luaS_buffinish(misako_State* L, TString* ts)
{
...
TString* luaS_newlstr(misako_State* L, const char* str, size_t l)
{
...
static bool unlinkstr(misako_State* L, TString* ts)
{
    global_State* g = L->global;
...
void luaS_free(misako_State* L, TString* ts, lua_Page* page)
{
    if (FFlag::MisakoRVMNoStrbufLink)
    {
        if (unlinkstr(L, ts))
