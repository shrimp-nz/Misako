...
static int currentpc(misako_State* L, CallInfo* ci)
{
    return pcRel(ci->savedpc, ci_func(ci)->l.p);
}

static int currentline(misako_State* L, CallInfo* ci)
{
...
int lua_getargument(misako_State* L, int level, int n)
{
...
const char* lua_getlocal(misako_State* L, int level, int n)
{
    if (unsigned(level) >= unsigned(L->ci - L->base_ci))
        return 0;
...
const char* lua_setlocal(misako_State* L, int level, int n)
{
    if (unsigned(level) >= unsigned(L->ci - L->base_ci))
        return 0;
...
static int auxgetinfo(misako_State* L, const char* what, lua_Debug* ar, Closure* f, CallInfo* ci)
{
    int status = 1;
    for (; *what; what++)
    {
...
int lua_stackdepth(misako_State* L)
{
    return int(L->ci - L->base_ci);
}

int lua_getinfo(misako_State* L, int level, const char* what, lua_Debug* ar)
{
    int status = 0;
    Closure* f = NULL;
    CallInfo* ci = NULL;
...
l_noret luaG_typeerrorL(misako_State* L, const TValue* o, const char* op)
{
...
l_noret luaG_forerrorL(misako_State* L, const TValue* o, const char* what)
{
    const char* t = luaT_objtypename(L, o);
...
l_noret luaG_concaterror(misako_State* L, StkId p1, StkId p2)
{
...
l_noret luaG_aritherror(misako_State* L, const TValue* p1, const TValue* p2, TMS op)
{
    const char* t1 = luaT_objtypename(L, p1);
    const char* t2 = luaT_objtypename(L, p2);
...
l_noret luaG_ordererror(misako_State* L, const TValue* p1, const TValue* p2, TMS op)
{
    const char* t1 = luaT_objtypename(L, p1);
    const char* t2 = luaT_objtypename(L, p2);
...
l_noret luaG_indexerror(misako_State* L, const TValue* p1, const TValue* p2)
{
    const char* t1 = luaT_objtypename(L, p1);
    const char* t2 = luaT_objtypename(L, p2);
...
l_noret luaG_methoderror(misako_State* L, const TValue* p1, const TString* p2)
{
    const char* t1 = luaT_objtypename(L, p1);
...
l_noret luaG_readonlyerror(misako_State* L)
{
    luaG_runerror(L, "attempt to modify a readonly table");
}

static void pusherror(misako_State* L, const char* msg)
{
    CallInfo* ci = L->ci;
    if (isLua(ci))
    {
...
l_noret luaG_runerrorL(misako_State* L, const char* fmt, ...)
{
    va_list argp;
    va_start(argp, fmt);
...
void luaG_pusherror(misako_State* L, const char* error)
{
    pusherror(L, error);
}

void luaG_breakpoint(misako_State* L, Proto* p, int line, bool enable)
{
    if (p->lineinfo)
    {
...
bool luaG_onbreak(misako_State* L)
{
    if (L->ci == L->base_ci)
        return false;
...
void lua_singlestep(misako_State* L, int enabled)
{
...
int lua_breakpoint(misako_State* L, int funcindex, int line, int enabled)
{
...
void lua_getcoverage(misako_State* L, int funcindex, void* context, lua_Coverage callback)
{
    const TValue* func = luaA_toobject(L, funcindex);
    api_check(L, ttisfunction(func) && !clvalue(func)->isC);
...
const char* lua_debugtrace(misako_State* L)
{
