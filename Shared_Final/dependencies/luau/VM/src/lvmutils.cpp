...
int luaV_tostring(misako_State* L, StkId obj)
{
    if (!ttisnumber(obj))
        return 0;
    else
...
static StkId callTMres(misako_State* L, StkId res, const TValue* f, const TValue* p1, const TValue* p2)
{
...
static void callTM(misako_State* L, const TValue* f, const TValue* p1, const TValue* p2, const TValue* p3)
{
    // using stack room beyond top is technically safe here, but for very complicated reasons:
    // * The stack guarantees EXTRA_STACK room beyond stack_last (see luaD_reallocstack) will be allocated
    // * we cannot move luaD_checkstack above because the arguments are *sometimes* pointers to the lua
...
void luaV_gettable(misako_State* L, const TValue* t, TValue* key, StkId val)
{
    int loop;
...
void luaV_settable(misako_State* L, const TValue* t, TValue* key, StkId val)
{
    int loop;
    TValue temp;
    for (loop = 0; loop < MAXTAGLOOP; loop++)
...
static int call_binTM(misako_State* L, const TValue* p1, const TValue* p2, StkId res, TMS event)
{
...
static const TValue* get_compTM(misako_State* L, Table* mt1, Table* mt2, TMS event)
{
...
static int call_orderTM(misako_State* L, const TValue* p1, const TValue* p2, TMS event)
{
...
int luaV_lessthan(misako_State* L, const TValue* l, const TValue* r)
{
    int res;
    if (ttype(l) != ttype(r))
...
int luaV_lessequal(misako_State* L, const TValue* l, const TValue* r)
{
    int res;
    if (ttype(l) != ttype(r))
...
int luaV_equalval(misako_State* L, const TValue* t1, const TValue* t2)
{
    const TValue* tm;
...
void luaV_concat(misako_State* L, int total, int last)
{
    do
    {
...
void luaV_doarith(misako_State* L, StkId ra, const TValue* rb, const TValue* rc, TMS op)
{
    TValue tempb, tempc;
    const TValue *b, *c;
...
void luaV_dolen(misako_State* L, StkId ra, const TValue* rb)
{
