...
// This header contains the bytecode definition for MisakoRVM interpreter
...
// range (indicated by LBC_BYTECODE_MIN / LBC_BYTECODE_MAX) and was produced by MisakoRVM compiler, it should load and execute correctly.
//
// Note that MisakoRVM runtime doesn't provide indefinite bytecode compatibility: support for older versions gets removed over time. As such, bytecode isn't a durable storage format and it's expected
// that MisakoRVM users can recompile bytecode from source on MisakoRVM version upgrades if necessary.
...
enum MisakoRVMOpcode
{
    ...
    // A: builtin function id (see MisakoRVMBuiltinFunction)
    ...
    // A: capture type, see MisakoRVMCaptureType
