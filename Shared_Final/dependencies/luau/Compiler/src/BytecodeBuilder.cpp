...
LUAU_FASTFLAGVARIABLE(MisakoRVMCompileBytecodeV3, false)
...
static int getOpLength(MisakoRVMOpcode op)
{
    switch (op)
    {
    case LOP_GETGLOBAL:
...
inline bool isJumpD(MisakoRVMOpcode op)
{
    switch (op)
    {
    case LOP_JUMP:
...
inline bool isSkipC(MisakoRVMOpcode op)
{
    switch (op)
    {
    case LOP_LOADB:
...
inline bool isFastCall(MisakoRVMOpcode op)
{
    switch (op)
    {
    case LOP_FASTCALL:
...
{
    MisakoRVMOpcode op = MisakoRVMOpcode(LUAU_INSN_OP(insn));
...
void BytecodeBuilder::emitABC(MisakoRVMOpcode op, uint8_t a, uint8_t b, uint8_t c)
{
...
void BytecodeBuilder::emitAD(MisakoRVMOpcode op, uint8_t a, int16_t d)
{
...
void BytecodeBuilder::emitE(MisakoRVMOpcode op, int32_t e)
{
...
bool BytecodeBuilder::patchJumpD(size_t jumpLabel, size_t targetLabel)
{
    ...
    LUAU_ASSERT(isJumpD(MisakoRVMOpcode(LUAU_INSN_OP(jumpInsn))));
...
bool BytecodeBuilder::patchSkipC(size_t jumpLabel, size_t targetLabel)
{
    ...
    LUAU_ASSERT(isSkipC(MisakoRVMOpcode(LUAU_INSN_OP(jumpInsn))) || isFastCall(MisakoRVMOpcode(LUAU_INSN_OP(jumpInsn))));
...
void BytecodeBuilder::writeFunction(std::string& ss, uint32_t id) const
{
    ...
    for (size_t i = 0; i < insns.size();)
    {
        ...
        int oplen = getOpLength(MisakoRVMOpcode(op));
        //uint8_t openc = encoder ? encoder->encodeOp(op) : op;
        uint8_t openc = op * 227;
...
{
    if (FFlag::MisakoRVMCompileBytecodeV3)
        return 3;
...
#define VJUMP(v) LUAU_ASSERT(size_t(i + 1 + v) < insns.size() && insnvalid[i + 1 + v])
    ...
    for (size_t i = 0; i < insns.size();)
    {
        ...
        i += getOpLength(MisakoRVMOpcode(op));
...
std::string BytecodeBuilder::dumpCurrentFunction() const
{
    ...
    for (size_t i = 0; i < insns.size();)
    {
        ...
        i += getOpLength(MisakoRVMOpcode(LUAU_INSN_OP(insns[i])));
        LUAU_ASSERT(i <= insns.size());
    }
