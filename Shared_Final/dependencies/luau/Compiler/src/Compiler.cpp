...
LUAU_FASTINTVARIABLE(MisakoRVMCompileLoopUnrollThreshold, 25)
LUAU_FASTINTVARIABLE(MisakoRVMCompileLoopUnrollThresholdMaxBoost, 300)

LUAU_FASTINTVARIABLE(MisakoRVMCompileInlineThreshold, 25)
LUAU_FASTINTVARIABLE(MisakoRVMCompileInlineThresholdMaxBoost, 300)
LUAU_FASTINTVARIABLE(MisakoRVMCompileInlineDepth, 5)

LUAU_FASTFLAGVARIABLE(MisakoRVMCompileXEQ, false)

LUAU_FASTFLAG(MisakoRVMInterpolatedStringBaseSupport)

LUAU_FASTFLAGVARIABLE(MisakoRVMCompileOptimalAssignment, false)

LUAU_FASTFLAGVARIABLE(MisakoRVMCompileExtractK, false)

namespace MisakoRVM
{

using namespace MisakoRVM::Compile;
...
struct Compiler
{
    ...
    void compileExprFastcallN(AstExprCall* expr, uint8_t target, uint8_t targetCount, bool targetTop, bool multRet, uint8_t regs, int bfid, int bfK = -1)
    {
        ...
        MisakoRVMOpcode opc = expr->args.size == 1 ? LOP_FASTCALL1 : LOP_FASTCALL2;

        if (FFlag::MisakoRVMCompileExtractK)
        {
    ...
    void compileExprCall(AstExprCall* expr, uint8_t target, uint8_t targetCount, bool targetTop = false, bool multRet = false)
    {
        ...
        if (options.optimizationLevel >= 2 && !expr->self)
        {
            ...
            if (fi && fi->canInline &&
                tryCompileInlinedCall(expr, func, target, targetCount, multRet, FInt::MisakoRVMCompileInlineThreshold,
                    FInt::MisakoRVMCompileInlineThresholdMaxBoost, FInt::MisakoRVMCompileInlineDepth))
                return;
        ...
        if (FFlag::MisakoRVMCompileExtractK && bfid == LBF_BIT32_EXTRACT && expr->args.size == 3 && isConstant(expr->args.data[1]) && isConstant(expr->args.data[2]))
        {
    ...
    MisakoRVMOpcode getUnaryOp(AstExprUnary::Op op)
    {
        switch (op)
        {
    ...
    MisakoRVMOpcode getBinaryOpArith(AstExprBinary::Op op, bool k = false)
    {
        switch (op)
        {
    ...
    MisakoRVMOpcode getJumpOpCompare(AstExprBinary::Op op, bool not_ = false)
    {
        switch (op)
        {
    ...
    size_t compileCompareJump(AstExprBinary* expr, bool not_ = false)
    {
        ...
        if (FFlag::MisakoRVMCompileXEQ)
        {
            ...
            {
                MisakoRVMOpcode opc = getJumpOpCompare(expr->op, not_);
    ...
    void compileExpr(AstExpr* node, uint8_t target, bool targetTemp = false)
    {
        ...
        else if (AstExprInterpString* interpString = node->as<AstExprInterpString>(); FFlag::MisakoRVMInterpolatedStringBaseSupport && interpString)
        {
    ...
    void compileStatFor(AstStatFor* stat)
    {
        ...
        if (options.optimizationLevel >= 2 && isConstant(stat->to) && isConstant(stat->from) && (!stat->step || isConstant(stat->step)))
            if (tryCompileUnrolledFor(stat, FInt::MisakoRVMCompileLoopUnrollThreshold, FInt::MisakoRVMCompileLoopUnrollThresholdMaxBoost))
                return;
    ...
    void compileStatForIn(AstStatForIn* stat)
    {
        ...
        MisakoRVMOpcode skipOp = LOP_FORGPREP;
    ...
    {
        LUAU_ASSERT(!FFlag::MisakoRVMCompileOptimalAssignment);
    ...
    void compileStatAssign(AstStatAssign* stat)
    {
        ...
        if (FFlag::MisakoRVMCompileOptimalAssignment)
        {
...
void compileOrThrow(BytecodeBuilder& bytecode, const ParseResult& parseResult, const AstNameTable& names, const CompileOptions& inputOptions)
{
    ...
    AstExprFunction main(root->location, /*generics= */ AstArray<AstGenericType>(), /*genericPacks= */ AstArray<AstGenericTypePack>(),
        /* self= */ nullptr, AstArray<AstLocal*>(), /* vararg= */ true, /* varargLocation= */ MisakoRVM::Location(), root, /* functionDepth= */ 0,
        /* debugname= */ AstName());
...
std::string compile(const std::string& source, const CompileOptions& options, const ParseOptions& parseOptions, BytecodeEncoder* encoder)
{
    ...
    if (!result.errors.empty())
    {
        ...
        const MisakoRVM::ParseError& parseError = result.errors.front();
