...
LUAU_FASTINTVARIABLE(MisakoRVMRecursionLimit, 1000)
LUAU_FASTINTVARIABLE(MisakoRVMParseErrorLimit, 100)

LUAU_FASTFLAGVARIABLE(MisakoRVMFixNamedFunctionParse, false)
LUAU_DYNAMIC_FASTFLAGVARIABLE(LuaReportParseWrongNamedType, false)

bool lua_telemetry_parsed_named_non_function_type = false;

LUAU_FASTFLAGVARIABLE(MisakoRVMErrorDoubleHexPrefix, false)
LUAU_FASTFLAGVARIABLE(MisakoRVMLintParseIntegerIssues, false)
LUAU_DYNAMIC_FASTFLAGVARIABLE(LuaReportParseIntegerIssues, false)

LUAU_FASTFLAGVARIABLE(MisakoRVMInterpolatedStringBaseSupport, false)
...
AstTypeOrPack Parser::parseFunctionTypeAnnotation(bool allowPack)
{
    ...
    if (FFlag::MisakoRVMFixNamedFunctionParse && !names.empty())
...
{
    LUAU_ASSERT(!FFlag::MisakoRVMLintParseIntegerIssues);
...
static ConstantNumberParseResult parseDouble(double& result, const char* data)
{
    ...
    if (data[0] == '0' && (data[1] == 'x' || data[1] == 'X') && data[2])
    {
        if (!FFlag::MisakoRVMErrorDoubleHexPrefix && data[2] == '0' && (data[3] == 'x' || data[3] == 'X'))
        {
            if (DFFlag::LuaReportParseIntegerIssues)
                lua_telemetry_parsed_double_prefix_hex_integer = true;
...
AstExpr* Parser::parseSimpleExpr()
{
    ...
    else if (lexer.current().type == Lexeme::RawString || lexer.current().type == Lexeme::QuotedString || (FFlag::MisakoRVMInterpolatedStringBaseSupport && lexer.current().type == Lexeme::InterpStringSimple))
    {
        return parseString();
    }
    else if (FFlag::MisakoRVMInterpolatedStringBaseSupport && lexer.current().type == Lexeme::InterpStringBegin)
    {
...
AstExpr* Parser::parseNumber()
{
    ...
    if (FFlag::MisakoRVMLintParseIntegerIssues)
    {
        double value = 0;
...
void Parser::incrementRecursionCounter(const char* context)
{
    ...
    if (recursionCounter > unsigned(FInt::MisakoRVMRecursionLimit))
    {
...
void Parser::report(const Location& location, const char* format, va_list args)
{
    ...
    if (FInt::MisakoRVMParseErrorLimit == 1)
        throw ParseError(location, message);

    parseErrors.emplace_back(location, message);

    if (parseErrors.size() >= unsigned(FInt::MisakoRVMParseErrorLimit))
        ParseError::raise(location, "Reached error limit (%d)", int(FInt::MisakoRVMParseErrorLimit));
}
