...
LUAU_FASTFLAG(DebugMisakoRVMTimeTracing)
...
struct Scope
{
    ...
    {
        if (!FFlag::DebugMisakoRVMTimeTracing)
            return;
...
#define LUAU_TIMETRACE_SCOPE(name, category) \
    static uint16_t lttScopeStatic = MisakoRVM::TimeTrace::createScopeData(name, category); \
    MisakoRVM::TimeTrace::Scope lttScope(lttScopeStatic)
...
#define LUAU_TIMETRACE_OPTIONAL_TAIL_SCOPE(name, category, microsec) \
    static uint16_t lttScopeStaticOptTail = MisakoRVM::TimeTrace::createScopeData(name, category); \
    MisakoRVM::TimeTrace::OptionalTailScope lttScope(lttScopeStaticOptTail, microsec)
